...
from blood_absorption.tasks.pouring.fluid_object import FluidObject, FluidObjectCfg
from blood_absorption.tasks.pouring.movement_env.suction_controller_no_timer import SuctionControllerNoTimer


@configclass
class PsmMovementLowDimEnvCfg(DirectRLEnvCfg):
    # 环境配置
    episode_length_s = 10
    decimation = 2
    action_space = 3
...
class PsmMovementLowDimEnv(DirectRLEnv):
    """低维观测的 PSM 粒子吸取环境."""

    cfg: PsmMovementLowDimEnvCfg

    def __init__(self, cfg: PsmMovementLowDimEnvCfg, render_mode: str | None = None, **kwargs):
